...
class CliffordAlgebra(nn.Module):
    ...
    def geometric_product(self, a, b, blades=None):
        ...
        if blades is not None:
            ...
            # Ensure index tensors are on the same device as the cayley tensor
            device = cayley.device
            if blades_l.device != device:
                blades_l = blades_l.to(device)
            if blades_o.device != device:
                blades_o = blades_o.to(device)
            if blades_r.device != device:
                blades_r = blades_r.to(device)
    ...
    def alpha(self, mv, blades=None):
        ...
        if blades is not None:
            if isinstance(blades, torch.Tensor) and blades.device != signs.device:
                blades = blades.to(signs.device)
            signs = signs[blades]
