...
#SBATCH --time=05:00:00                 # Максимальное время выполнения
#SBATCH --ntasks-per-node=5          # Количество MPI процессов на узел
#SBATCH --nodes=1                    # Требуемое кол-во узлов
#SBATCH --gpus-per-node=4                     # Требуемое кол-во GPU
...
singularity exec --nv $BIND_OPTS "$IMAGE" \
  bash $WORKDIR/scripts/top_tagging/B_TT_Nruns.sh "$N_RUNS" "$DATA_ROOT"
