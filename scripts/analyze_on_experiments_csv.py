#!/usr/bin/env python3
"""
Simpler script to analyze ON Q5 experiments using CSV logs.
This is a backup option if TensorBoard logs are not available.
"""
...
def extract_metrics_from_csv(log_dir: str, run_name: str) -> Dict[str, List[float]]:
    """
    Extract metrics from CSV log files for a specific run.
    
    Args:
        log_dir: Directory containing CSV logs
    ...
    for version_dir in version_dirs:
        csv_files = glob.glob(os.path.join(version_dir, "metrics.csv"))
        if not csv_files:
            continue
        ...
        try:
            ...
            # Extract all numeric columns (metrics)
            for column in df.columns:
                if column not in ['epoch', 'step'] and df[column].dtype in ['float64', 'int64']:
                    values = df[column].dropna().tolist()
                    if values:
                        if column not in metrics:
                            metrics[column] = []
                        metrics[column].extend(values)
...
def analyze_experiments(log_dir: str = "lightning_logs", output_file: str = None):
    ...
    """
    print("Analyzing ON Q5 experiments using CSV logs...")
    ...
    for exp_key, exp_description in expected_experiments:
        ...
        for run_name in runs:
            ...
            run_metrics = extract_metrics_from_csv(log_dir, run_name)
        ...
        for metric_name, values in all_metrics.items():
            ...
            # Print key metrics
            if 'loss' in metric_name.lower() or 'accuracy' in metric_name.lower():
                print(f"    {metric_name}:")
...
def main():
    parser = argparse.ArgumentParser(description="Analyze ON Q5 experiment results using CSV logs")
